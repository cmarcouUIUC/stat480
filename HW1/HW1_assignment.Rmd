...
At q=.01, there are 290 discoveries. 

```{r}
table(mrgpvals<=cutoff) # number of discoveries and non-discoveries

#the betas aren't saved ?
t<-table(beta,mrgpvals<=cutoff)
t[1,2]/(sum(t[,2])) #get False Discovery Proportion
```
...
```{r}
tail(mrgpvals_ordered, 10) #select 10 most significant words
```
