...
#topic modeling
library(maptpx)
x <- as.simple_triplet_matrix(dtm_2)
...
x <- 100*dtm/rowSums(dtm)
regwords.cv <- cv.gamlr(dtm, drugsComTrain_raw$rating)
##regress words vs useful count
regwords.cv.2 <- cv.gamlr(dtm, drugsComTrain_raw$usefulCount)
```
