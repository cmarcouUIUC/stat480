...
regwords.cv <- cv.gamlr(x.words, train$rating)
ratingreg <- gamlr(tpcs$omega, train$rating,family="binomial") #topic regression
...
regclust <- cv.gamlr(xclust,###wine$quality,lambda.min.ratio=1e-5) # 
plot(regclust)
max(1-regclust$cvm/regclust$cvm[1]) # OOS R2 around 0.22
```
